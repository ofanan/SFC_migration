...
class Res_file_parser (object):
    ...
    def set_plt_params (self, size='large'):
        ...
        if self.useLatex:
            matplotlib.rcParams.update({
                ...
                "font.serif"        : ["Computer Modern Roman"], # Use a specific serif font
                ...
                'text.usetex'       : True,
                "text.latex.preamble": r"\usepackage{amsmath}", # Example: include amsmath
                'pgf.rcfonts'       : False})
        else:
            mpl.rcParams.update({
                'font.size'         : FONT_SIZE         [size],
    ...
    def plot_RT_prob_sim_python (
            ...
            plot_overall_cpu    = False,# When True, plot the overall cpu. When False, plot the CPU at a leaf node.  
            legend_at_top       = False,
        ):
        """
        ...
        full_path_output_file_name = '../res/{}{}{}' .format (
            'dist_' if dist else '', 
            input_file_name,
    ...
    def gen_cost_vs_rsrc_tbl (self, city, normalize_X = True, slot_len_in_sec=1, normalize_Y=True, dist=True, pcl_input_file_name=None):
        ...
        SyncPartReshData = [item for item in self.cost_vs_rsrc_data if item['mode']=='SyncPartResh']
        # print ('SyncPartReshData={}' .format (SyncPartReshData)) #$$$
        # exit ()
        # printf ('../res/gamad.txt', 'SyncPartReshData={}' .format (SyncPartReshData)) #$$$
        # print ('len of SyncPartReshData={}' .format (len(SyncPartReshData))) #$$$
        ...
        printf (self.output_file, '\n')
        preferred_cpu_vals = [1060, 1260, 1329, 1680] if city=='Monaco' else [118, 157, 178, 209, 214]
        #cpu_vals = list (set(cpu_vals) & set(preferred_cpu_vals))
        print ('**** Note: you chose to show results only for some preferred cpu vals.***')
        min_cpu  = min (cpu_vals)
        for cpu_val in sorted (cpu_vals):
            if (normalize_Y):
    ...
    def calc_cost_vs_rsrc (self, pcl_input_file_name=None, res_input_file_names=None, min_t=30001, max_t=30600, prob=0.3, dist=True):
        ...
        * Returns the file name to which it saved the pickled results. 
        ...
        modes = ['AsyncNBlk', 'opt', 'SyncPartResh'] if dist else ['opt', 'optG', 'optInt', 'ourAlg', 'ms', 'ffit', 'cpvnf', 'SyncPartResh']   
        for mode in modes:
