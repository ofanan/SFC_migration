...
class SFC_mig_simulator (object):
    ...
    s_has_sufic_avail_cpu_for_u = lambda self, s, u : (self.G.nodes[s]['a'] >= usr.B[self.G.nodes[s]['lvl']])
    ...
    def first_Fit_place_usr (self, usr):
        ...
        for s in reversed(usr.S_u):
            if (self.s_has_sufic_avail_cpu_for_u (s, u)): # if the available cpu at this server > the required cpu for this usr at this lvl...
                usr.nxt_s = s
...
if __name__ == "__main__":
    ...
    my_simulator.simulate (alg          - 'ffit', #algorithm to simulate 
