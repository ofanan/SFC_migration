...
class SFC_mig_simulator (object):
    ...
    def run_prob_of_RT_sim_algs (self):
        ...
        for seed in [40 + delta_sd for delta_sd in range (6) ]:
            for prob_of_target_delay in [i/10 for i in range (3)]:
...
def run_cost_by_rsrc (poa_file_name, poa2cell_file_name, seeds=None):
    ...
    for cpu_cap_at_leaf in [inter (min_req_cpu[my_simulator.city]['opt']*(1 + i/10)) for i in range(1, 5)]: # simulate for opt's min cpu * [100%, 110%, 120%, ...]
