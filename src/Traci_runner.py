...
VERBOSE_LOC_FILE      = 1
VERBOSE_LOC_AND_DEBUG = 2

class Traci_runner (object):

    
    def __init__ (self, warmup_period=0, sim_length=10, num_of_output_files=1, verbose = VERBOSE_LOC_FILE):
        traci.start([checkBinary('sumo'), '-c', 'my.sumocfg', '-W', '-V', 'false', '--no-step-log', 'true']) 
        warmup_period           = 0 #3600*7.5
        sim_length              = 10 #3600*1.5
        len_of_time_slot_in_sec = 1 #60  
...
if __name__ == '__main__':
    my_Traci_runner = Traci_runner (warmup_period=0, sim_length=10, num_of_output_files=1, verbose = VERBOSE_LOC_FILE)
